...
async function main() {
  ...
  console.log(
    'Selos e 7 doações (1 aprovada, 1 reprovada, 5 pendentes) criadas com sucesso!',
  );
}
...
main()
  .then(() => {
    console.log(
      'Seed finalizado com sucesso. Banco de dados populado do zero.',
    );
  })
  .catch((e) => {
    console.error('Ocorreu um erro durante o processo de seed:', e);
    // rethrow para CI/Chamador saber que falhou (não usar process.exit aqui)
    throw e;
  })
  .finally(async () => {
