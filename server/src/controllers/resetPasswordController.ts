...
import { resetPasswordEmail } from '../services/mailService';
...
class PasswordController {
    ...
    public forgotPassword = async (req: Request, res: Response): Promise<Response> => {
        ...
        try {
            ...
            try {
                await resetPasswordEmail(
                  ...
                  `Você solicitou a redefinição de senha. Clique neste link ${resetURL}`
                );
     ...
     public resetPassword = async (req: Request, res: Response): Promise<Response> => {
      ...
      try {
        const user = await UserRepository.findByResetToken(token as string);
