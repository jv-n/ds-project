...
export default function DetalhesPontuacao() {
  ...
  const pontuacoes = [
    {
      label: "Ações de Conscientização e Educação Interna sobre ODSS",
      detalhe: "6 ações realizadas",
      pontos: "25 pts",
    },
    {
      label: "ODSs com Atuação da Empresa",
      detalhe: "7 a 8 ODS abordados",
      pontos: "20 pts",
    },
    {
      label: "ONGs Atingidas por Ações de Voluntariado",
      detalhe: "5 a 6 ONGs parceiras",
      pontos: "15 pts",
    },
    {
      label: "Colaboradores Engajados em Ações de Voluntariado",
      detalhe: "6% a 10% dos colaboradores",
      pontos: "4 pts",
    },
    {
      label: "Orçamento Destinado a Voluntariado e Iniciativas Sociais",
      detalhe: "Até 0.1% do orçamento",
      pontos: "4 pts",
    },
  ];
  ...
  return (
    <div className="w-full max-w-4xl bg-white rounded-xl shadow-lg mb-8 overflow-hidden">
      ...
      >
        <h3 className="text-xl font-bold text-[#1B2029]">
      ...
      {isOpen && (
        <div className="p-6">
          {pontuacoes.map((item, index) => (
            <div
              key={index}
              className="flex justify-between items-center py-3 border-b last:border-b-0 border-gray-100"
            >
              ...
              <span className="font-bold text-[#1B2029] text-sm">
                {item.pontos}
              </span>
            </div>
          ))}
