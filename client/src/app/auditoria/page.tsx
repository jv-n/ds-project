...
export default function AuditoriaPage() {
    ...
    try {
      ...
      const dadosMapeadosPromises = doacoesIncompletas.map(async (item) => {
        try {
          ...
          const acaoCompleta = todasAcoesDaEmpresa.find(
            (acao: { acaoId: any }) => acao.acaoId === item.acaoId
          );
          ...
          const emailDaEmpresa =
            dadosDaEmpresa?.usuario?.email ?? "Email não informado";
          ...
          return {
            ...
            valorDoacao: new Intl.NumberFormat("pt-BR", {
              style: "currency",
              currency: "BRL",
            }).format(item.valor),
      ...
      const dadosMapeados = (await Promise.all(dadosMapeadosPromises)).filter(
        Boolean
      );
  ...
  const handleUpdateStatus = (newStatus: "aprovada" | "reprovada") => {
    ...
    setDadosAuditoria((currentData) =>
      currentData.map((item) =>
        item.id === selectedAuditoria.id ? { ...item, status: newStatus } : item
      )
    );
  };
  ...
  const filteredAuditorias = dadosAuditoria.filter((auditoria) => {
    ...
    const searchMatch =
      (auditoria.nomeEmpresa?.toLowerCase() ?? "").includes(
        searchTerm.toLowerCase()
      ) ||
      (auditoria.nomeONG?.toLowerCase() ?? "").includes(
        searchTerm.toLowerCase()
      );
